...
if(!empty($_POST) && check_admin_referer('pronamic_ideal_save_configuration', 'pronamic_ideal_nonce')) {
	...
	$variant = ConfigurationsRepository::getVariantById($variantId);
	...
	// Update
	$updated = ConfigurationsRepository::updateConfiguration($configuration);

	// Transient
	IDeal::deleteConfigurationTransient($configuration);

	if($updated) {
		$update = sprintf(
			__('Configuration updated, %s.', Plugin::TEXT_DOMAIN) , 
			sprintf('<a href="%s">', Admin::getConfigurationsLink()) . __('back to overview', Plugin::TEXT_DOMAIN) . '</a>'
		);
	}
}
...
<div class="wrap">
	...
	<form enctype="multipart/form-data" action="" method="post">
			...
			<tr>
				<th scope="row">
					<label for="pronamic_ideal_variant_id">
						<?php _e('Variant', Plugin::TEXT_DOMAIN); ?>
					</label>
				</th>
				<td>
	                ...
	                <select id="pronamic_ideal_variant_id" name="pronamic_ideal_variant_id">
	                	<option value=""></option>
	                	<?php foreach(ConfigurationsRepository::getProviders() as $provider): ?>
			...
			<tr>
				<th scope="row">
					<label for="pronamic_ideal_merchant_id">
						<?php _e('Merchant ID', Plugin::TEXT_DOMAIN); ?>
					</label>
				</th>
				<td>
					...
					<span class="description">
						<br />
						<?php _e('You receive the merchant ID (also known as: acceptant ID) from your iDEAL provider.', Plugin::TEXT_DOMAIN); ?>
					</span>
				</td>
			</tr>
			<tr>
				<th scope="row">
					<label for="pronamic_ideal_sub_id">
						<?php _e('Sub ID', Plugin::TEXT_DOMAIN); ?>
					</label>
				</th>
				<td>
					...
					<span class="description">
						<br />
						<?php printf(__('You receive the sub ID from your iDEAL provider, the default is: %s.', Plugin::TEXT_DOMAIN), 0); ?>
					</span>
				</td>
			</tr>
			<tr>
				<th scope="row">
					<label for="pronamic_ideal_mode">
						<?php _e('Mode', Plugin::TEXT_DOMAIN); ?>
					</label>
				</th>
				<td>
					<fieldset>
						...
						<p>		
							<label>
								<input type="radio" value="<?php echo IDealCore::MODE_LIVE; ?>" name="pronamic_ideal_mode" <?php checked($configuration->mode, IDealCore::MODE_LIVE); ?> />
								<?php _e('Live', Plugin::TEXT_DOMAIN); ?>
							</label><br />
			
							<label>
								<input type="radio" value="<?php echo IDealCore::MODE_TEST; ?>" name="pronamic_ideal_mode" <?php checked($configuration->mode, IDealCore::MODE_TEST); ?> />
		...
		<h3>
			<?php _e('Basic', Plugin::TEXT_DOMAIN); ?>
		</h3>
			...
			<tr>
				<th scope="row">
					<label for="pronamic_ideal_hash_key">
						<?php _e('Hash Key', Plugin::TEXT_DOMAIN); ?>
					</label>
				</th>
				<td>
		...
		<h3>
			<?php _e('Advanced', Plugin::TEXT_DOMAIN); ?>
		</h3>
			...
			<tr>
				<th scope="row">
					<label for="pronamic_ideal_private_key_password">
						<?php _e('Private Key Password', Plugin::TEXT_DOMAIN); ?>
					</label>
				</th>
				<td> 
			...
			<tr>
				<th scope="row">
					<label for="pronamic_ideal_private_key">
						<?php _e('Private Key', Plugin::TEXT_DOMAIN); ?>
					</label>
				</th>
				<td>
			...
			<tr>
				<th scope="row">
					<label for="pronamic_ideal_private_certificate">
						<?php _e('Private Certificate', Plugin::TEXT_DOMAIN); ?>
					</label>
				</th>
				<td>
					...
					if(!empty($configuration->privateCertificate)) {
						$fingerprint = Security::getShaFingerprint($configuration->privateCertificate);
						$fingerprint = str_split($fingerprint, 2);
						$fingerprint = implode(':', $fingerprint);
					
						echo sprintf(__('SHA Fingerprint: %s', Plugin::TEXT_DOMAIN), $fingerprint), '<br />';
					}
