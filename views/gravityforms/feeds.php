...
<div class="wrap">
	...
	<h2>
		...
		if(true): ?>
			...
			<a class="button add-new-h2" href="<?php echo Pronamic_GravityForms_IDeal_Admin::get_edit_feed_link(); ?>">
	...
	if ( ! empty( $_POST ) && check_admin_referer( 'pronamic_ideal_search_lead', 'pronamic_ideal_nonce' ) ) {
		...
		$leads_table = $wpdb->prefix . 'rg_lead';

		$results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $leads_table WHERE id = %s", $id ) );
	}
