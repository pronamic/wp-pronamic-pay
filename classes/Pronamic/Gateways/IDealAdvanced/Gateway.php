...
class Pronamic_Gateways_IDealAdvanced_Gateway extends Pronamic_Gateways_Gateway {
	...
	public function start( Pronamic_Pay_PaymentDataInterface $data, Pronamic_Pay_Payment $payment ) {
		...
		if ( $error !== null ) {
			...
			update_post_meta( $payment->id, '_pronamic_payment_authentication_url', $result->issuer->authenticationUrl );
			update_post_meta( $payment->id, '_pronamic_payment_transaction_id', $result->transaction->getId() );
		}
	}
	...
	public function payment( Pronamic_Pay_Payment $payment ) {
		...
		wp_schedule_single_event( $time +    30, 'pronamic_ideal_check_transaction_status', array( 'payment_id' => $payment->id, 'seconds' =>    30 ) );
		// Half-way through an expirationPeriod
		wp_schedule_single_event( $time +  1800, 'pronamic_ideal_check_transaction_status', array( 'payment_id' => $payment->id, 'seconds' =>  1800 ) );
		// Just after an expirationPeriod
		wp_schedule_single_event( $time +  3600, 'pronamic_ideal_check_transaction_status', array( 'payment_id' => $payment->id, 'seconds' =>  3600 ) );
		// A certain period after the end of the expirationPeriod
		wp_schedule_single_event( $time + 86400, 'pronamic_ideal_check_transaction_status', array( 'payment_id' => $payment->id, 'seconds' => 86400 ) );
	}
